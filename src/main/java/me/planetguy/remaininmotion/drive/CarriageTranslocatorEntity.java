...
public class CarriageTranslocatorEntity extends CarriageDriveEntity {
	...
	public CarriagePackage PreparePackage(Directions MotionDirection) throws CarriageMotionException {
		...
		for (int Index = 0; Index < ActiveTranslocators.size(); Index++) {
			...
			try {
				CarriageTranslocatorEntity Translocator = (CarriageTranslocatorEntity) WorldUtil.GetWorld(
	...
	public void EstablishSpectre(CarriagePackage Package) {
		WorldUtil.SetBlock(worldObj, Package.AnchorRecord.X, Package.AnchorRecord.Y, Package.AnchorRecord.Z,
				RIMBlocks.Spectre, Spectre.Types.Teleportative.ordinal());

		((TeleportativeSpectreEntity) worldObj.getTileEntity(Package.AnchorRecord.X, Package.AnchorRecord.Y,
		...
		WorldUtil.SetBlock(Package.Translocator.worldObj, NewX, NewY, NewZ, RIMBlocks.Spectre,
				Spectre.Types.Teleportative.ordinal());

		((TeleportativeSpectreEntity) Package.Translocator.worldObj.getTileEntity(NewX, NewY, NewZ))
				.AbsorbSink(Package);
	}
}
