...
public abstract class CarriageDriveEntity extends TileEntity implements IEnergyHandler {
	...
	public void HandleNeighbourBlockChange() {
		...
		for (Directions Direction : Directions.values()) {
			...
			if (m != null) if (CarriageDirection != null) {
	...
	public void removeUsedEnergy(CarriagePackage _package) throws CarriageMotionException {
		...
		if (Configuration.HardmodeActive) {
			...
			{
				double MaxBurden = CarriageDrive.Types.values()[Type].MaxBurden
						* CarriageDrive.Tiers.values()[Tier].MaxBurdenFactor;
			...
			double EnergyRequired = _package.getMass() * CarriageDrive.Types.values()[Type].EnergyConsumption
					* CarriageDrive.Tiers.values()[Tier].EnergyConsumptionFactor;
	...
	public void EstablishSpectre(CarriagePackage Package) {
		...
		WorldUtil
				.SetBlock(worldObj, CarriageX, CarriageY, CarriageZ, RIMBlocks.Spectre, Spectre.Types.Motive.ordinal());
	...
	public int receiveEnergy(ForgeDirection from, int maxReceive, boolean simulate) {
		...
		if (!simulate) energyStored += toRecieve;
		return toRecieve;
	}
	...
	public IIcon getIcon(int Side, int meta) {
		try {
			if (SideClosed[Side]) { return (CarriageDrive.InactiveIcon); }
