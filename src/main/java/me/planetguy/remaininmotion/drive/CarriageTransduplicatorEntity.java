...
import me.planetguy.remaininmotion.spectre.TransduplicativeSpectreEntity;
...
public class CarriageTransduplicatorEntity extends CarriageTranslocatorEntity {
	...
	public CarriagePackage PreparePackage(Directions MotionDirection) throws CarriageMotionException {
		...
		for (int Index = 0; Index < ActiveTranslocators.size(); Index++) {
			...
			try {
				CarriageTransduplicatorEntity Translocator = (CarriageTransduplicatorEntity) WorldUtil.GetWorld(
	...
	public void EstablishSpectre(CarriagePackage Package) {
		WorldUtil.SetBlock(worldObj, Package.AnchorRecord.X, Package.AnchorRecord.Y, Package.AnchorRecord.Z,
				RIMBlocks.Spectre, Spectre.Types.Transduplicative.ordinal());

		((TransduplicativeSpectreEntity) worldObj.getTileEntity(Package.AnchorRecord.X, Package.AnchorRecord.Y,
		...
		WorldUtil.SetBlock(Package.Translocator.getWorldObj(), NewX, NewY, NewZ, RIMBlocks.Spectre,
				Spectre.Types.Transduplicative.ordinal());

		((TransduplicativeSpectreEntity) Package.Translocator.getWorldObj().getTileEntity(NewX, NewY, NewZ))
				.AbsorbSink(Package);
	}
}
